...
if ($_POST['status'] == 'insert') {
    ...
    if (!($chk->num_rows)) {
        ...
        if ($img['status']) {
            ...
            $query = "INSERT INTO `centers` (`id`, `timestamp`, `center_number`, `name`, `institute_name`, `dob`, `pan_number`, `aadhar_number`, `center_full_address`,`pincode`, `state_id`, `city_id`, `no_of_computer_operator`, `no_of_class_room`, `total_computer`, `space_of_computer_center`, `whatsapp_number`, `contact_number`, `email_id`, `qualification_of_center_head`, `staff_room`, `water_supply`, `toilet`, `reception`,  `transection_id`, `status`, `image`) VALUES (NULL, CURRENT_TIMESTAMP, '" . time() . "', '$name', '$institute_name', '$dob', '$pan_number', '$aadhar_number', '$center_full_address','$pincode', '" . $_POST['state_id'] . "', '" . $_POST['city_id'] . "', '" . $_POST['no_of_computer_operator'] . "', '" . $_POST['no_of_class_room'] . "', '" . $_POST['total_computer'] . "', '" . $_POST['space_of_computer_center'] . "', '$whatsapp_number', '" . $_POST['contact_number'] . "', '$email_id', '$qualification_of_center_head', '" . $_POST['staff_room'] . "', '" . $_POST['water_supply'] . "', '" . $_POST['toilet'] . "', '$reception', '', '1', '$img[file_name]')";

            $data = $con->query($query);

            if ($data) {
                echo '<script>alert("Registration Success,Please Contact in BIIM");location.href="franchisee_form.php"</script>';
            } else {
...
<div class="container">
	...
	<h1 class="text_heading text-center">FRANCHISE  <span class="highlight_color">FORM</span></h1>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('.get_city').change(function() {
            ...
            $.ajax({
                url: "admin/Ajax.php", // Update with the correct URL
                type: "POST",
                data: dataString,
                beforeSend: function() {
