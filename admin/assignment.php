...
if ($_POST['status'] == 'assignment') {
    $img = image_upload('image', 'assignment');
    if ($img['status']) {
        $image = $img['file_name'];
    } else {
        $image = $img['error'];
    ...
    if(!$fileName == ''){
    $con->query("INSERT INTO `assignment` (file, description) VALUE ('$fileName', '".$_POST['description']."')");
    echo '<script>alert("Assignment Uploaded Succesfully");window.location.href="assignment.php"</script>';
    }
    else{
...
<div class="panel panel-primary">
    ...
    <form method="post" action="" enctype="multipart/form-data">
        ...
        <div class="panel-footer">
            <button type="submit" class="btn btn-sm btn-success" name="status" value="assignment"><i class="fa fa-save"></i>Save</button>
        </div>
    </form>
</div>
							...
							if($get->num_rows){
							    ...
							    while($row = $get->fetch_assoc()){
							        echo '<tr>
							                ...
							                <td>
                                                <a href="download_assignment.php?file='.$row['file'].'" target="_blank" class="btn btn-success"><i class="fa fa-download"></i><a/>
                                                <a href="?action=delete&id='.$row['id'].'" class="btn btn-danger" onclick="return confirm(\'Are You Confirm parmanent Delete This student\')"><i class="fa fa-trash"></i></a>
                                            </td>
							              </tr>';
							    }
							}
